...
export function SavedUrlAccordion({ projectId, record, index, count, expanded, model, onToggle, onDeleted }: SavedUrlAccordionProps) {
    ...
    <article className="url-accordion" data-record-id={record.id}>
      ...
      {expanded && (
        <div id={`saved-url-${record.id}`} className="accordion-body">
          ...
          <div className="tag-display"><span>Tags</span>{record.tags.length ? <div>{record.tags.map((tag) => <span className="tag" key={tag}>{tag}</span>)}</div> : <em>No tags</em>}</div>
